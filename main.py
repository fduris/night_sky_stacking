...
# Star detection and alignment constants
STAR_DETECTION_SIGMA_CLIP = 3.0
...
OUTPUT_TYPE = OutputType.JPG
OUTPUT_FILENAME = "stacked_image"
...
def detect_stars(images: List[np.ndarray], mask: Optional[np.ndarray] = None) -> List[np.ndarray]:
    star_coords_list = []
    for img in images:
        ...
        mean, median, std = sigma_clipped_stats(
            ...
            sigma=STAR_DETECTION_SIGMA_CLIP,
        )
...
def compute_affine_transforms(star_coords_list: List[np.ndarray]) -> List[Optional[np.ndarray]]:
...
def save_image(image: np.ndarray, output_type: OutputType) -> None:
    if output_type == OutputType.TIFF:
        _save_image_as_tiff(image, filename=OUTPUT_FILENAME + ".tiff")
    elif output_type == OutputType.JPG:
        _save_image_as_jpg(image, filename=OUTPUT_FILENAME + ".jpg")
    else:
        raise RuntimeError(f"Unsupported output type {output_type}")


def _save_image_as_tiff(image: np.ndarray, filename: str) -> None:
    # Normalize the image to 16-bit unsigned integer
    image_uint16 = cv2.normalize(src=image, dst=np.ndarray([]), alpha=0, beta=65535, norm_type=cv2.NORM_MINMAX).astype(
        np.uint16
    )
...
def _save_image_as_jpg(image: np.ndarray, filename: str) -> None:
    # Normalize the image to 8-bit unsigned integer
    image_uint8 = cv2.normalize(src=image, dst=np.ndarray([]), alpha=0, beta=255, norm_type=cv2.NORM_MINMAX).astype(
        np.uint8
    )
    # Convert RGB to BGR for OpenCV
...
def main() -> None:
    ...
    affine_transforms: List[Optional[np.ndarray]] = compute_affine_transforms(star_coords_list)

    print("Applying affine transformations to color images...")
    aligned_images: List[np.ndarray] = apply_affine_transforms(raw_images, affine_transforms)
    ...
    save_image(stacked_image, output_type=OUTPUT_TYPE)
